...
const emailForm = (title, name, otherName, content, url) => {
    let string = `
<div style="width: 90%; border: 2px solid lightskyblue; margin: 1rem auto; padding: 1rem; text-align: center;">
        <p style="border-bottom: 1px dashed lightskyblue;margin: 0;padding-bottom: 1rem; color: lightgreen; font-size: 1.25rem;">${title}</p>
        <p style="margin: 1rem 0 0;">hello,${name} &#x1f608</p>
        <p style="margin: 0 0 1rem;">${otherName}回复了你的评论</p>
        ...
        if (doc && doc.address !== req.body.address) {
            ...
            if (/^@(.*):/.test(req.body.content)) {
                ...
                db.Comment.findOne({name: reviewer, articleId: req.body.articleId}, (err, doc) => {
                    ...
                    const content =  emailForm('欢迎常来我的博客', reviewer, req.body.name, req.body.content, url)
                    mail.send(replyEmail, '您在xxx博客有一条新评论', content, res)
                })
            }
            new db.Comment(comment).save().then(() => {
                const url = 'https://www.xxx.cn' + req.body.curPath
                const content = emailForm('MyBlog Message', '站长', req.body.name, req.body.content, url)
